...
export async function consultarParceiros(page: number = 1, pageSize: number = 50, searchName: string = '', searchCode: string = '') {
  // Calcular o offset baseado na página
  const offset = (page - 1) * pageSize;

  // Construir critério de busca - só aplica se houver filtro
  ...
  // Se houver filtros, junta com AND
  if (filters.length > 0) {
  ...
  const dataSet: any = {
    ...
    "entity": {
      "fieldset": {
        "list": "CODPARC, NOMEPARC, CGC_CPF, CODCID, ATIVO, TIPPESSOA, RAZAOSOCIAL, IDENTINSCESTAD, CEP, CODEND, NUMEND, COMPLEMENTO, CODBAI, LATITUDE, LONGITUDE"
      }
    }
  };

  // Só adiciona criteria se houver filtro
        ...
        "records": [
          {
            ...
            "values": {
              ...
              "4": parceiro.CGC_CPF,
              "5": parceiro.CODCID,
              "6": parceiro.RAZAOSOCIAL || "",
              "7": parceiro.IDENTINSCESTAD || "",
              "8": parceiro.CEP || "",
              "9": parceiro.CODEND || "",
              "10": parceiro.NUMEND || "",
              "11": parceiro.COMPLEMENTO || "",
              "12": parceiro.CODBAI || "",
              "13": parceiro.LATITUDE || "",
              "14": parceiro.LONGITUDE || ""
            }
          }
        ]
      }
